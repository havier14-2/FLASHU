...
public interface ProductoRepositories extends JpaRepository<Producto, Long> {
    ...
    @Query("SELECT p FROM Producto p LEFT JOIN FETCH p.categoria WHERE " +
           ...
           "(:stockMenorA IS NULL OR p.stock < :stockMenorA)") // <-- LÍNEA AÑADIDA
    Page<Producto> findWithFiltersAndPagination(String nombre, Long categoriaId, Integer stockMenorA, Pageable pageable);
